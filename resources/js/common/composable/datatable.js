...
const datatable = () => {
    ...
    const generateUrl = (limit, offset) => {
        ...
        if (table.searchColumn != undefined && table.searchString != "" && table.searchColumn != "full_name") {
            filterString += `${table.searchColumn} lk "%${table.searchString}%" and `;
            trimString = true;
            table.filterLoading = true;
        } else if (table.searchString != "" && table.filterableColumns.length > 0 && table.searchColumn != "full_name") {
            var newSearchString = "";
    ...
    const fetch = (params = {}) => {
        ...
        queryData({
            ...
            table.selectedRowKeys = table.selectedRowKeys.length > 0 ? table.selectedRowKeys : [];
